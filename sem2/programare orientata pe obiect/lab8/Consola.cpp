...
void ConsolUI::exporta(const vector<Meds>& meds) {
	ofstream g("reteta.txt");
	g << "Reteta:\n";
	for (const auto& med : meds) {
		g << ' ' << med.getName() << ' ' << med.getProducer() << ' ' << med.getActSubst() << ' ' << med.getPrice() << '\n';
	}
	g << "Sfarsit reteta\n";
	g.close();
}
