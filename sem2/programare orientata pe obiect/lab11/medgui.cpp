...
void PharmacyGUI::initGUICmps() {
	...
	btnAddtoReteta = new QPushButton("Add to prescription");
	formLDetalii->addWidget(btnAddtoReteta);

	lyMain->addWidget(widDetalii);

	QWidget* reteta = new QWidget;
	QFormLayout* layout = new QFormLayout;
	reteta->setLayout(layout);
	lst = new QListWidget;
	layout->addWidget(lst);

	btnAddRandom = new QPushButton("Add random to prescription");
	layout->addWidget(btnAddRandom);
...
void PharmacyGUI::connectSignalsSlots() {
	...
	QObject::connect(btnAddtoReteta, &QPushButton::clicked, this, &PharmacyGUI::addToReteta);
	...
	QObject::connect(btnAddRandom, &QPushButton::clicked, this, &PharmacyGUI::addRandom);

	QObject::connect(btnEmpty, &QPushButton::clicked, this, &PharmacyGUI::empty);

	QObject::connect(btnExport, &QPushButton::clicked, this, &PharmacyGUI::exportReteta);
...
void PharmacyGUI::addToReteta() {
	ctr.addToReteta(txtName->text().toStdString());
	reloadReteta(ctr.getMedsFromReteta());
}

void PharmacyGUI::addRandom() {
	...
	connect(btnGenerate, &QPushButton::clicked, [=] {
		ctr.addRandomToReteta(txtRandom->text().toInt());
		reloadReteta(ctr.getMedsFromReteta());
		});
}

void PharmacyGUI::empty() {
	ctr.emptyReteta();
	reloadReteta(ctr.getMedsFromReteta());
}

void PharmacyGUI::exportReteta() {
	QWidget* widget = new QWidget;
	...
	connect(btnsubmit, &QPushButton::clicked, [=] {
		ctr.exporta(txtFile->text().toStdString());
		});
}
...
void PharmacyGUI::reloadList(const std::vector<Meds>& meds) {
	...
	int lineNumber = 0;
	for (auto& med : meds) {
		this->tableMeds->setItem(lineNumber, 0, new QTableWidgetItem(QString::fromStdString(med.getName()+" " +med.getProducer())));
...
void PharmacyGUI::reloadReteta(const std::vector<Meds> meds) {
	lst->clear();
