...
/*
	...
	*/
void Pharmacy::deleteMed(string name) {
	Meds m = findMed(name);
	rep.del(m);
...
/*
	modifica un element
	*/
void Pharmacy::updateMed(string name, string newname, string newproducer, string newsubst, int newprice) {
	Meds m = findMed(name);
...
int Pharmacy::addRandomToReteta(int howMany) {
	RetetaCurenta.addRandomMeds(howMany);
