...
void PharmacyGUI::initGUICmps() {
	...
	lyFormFilter->addWidget(btnFilterPrice);
	...
	lyFormFilter->addWidget(btnFilterSubst);

	vl->addWidget(formFilter);
	...
	txtNameDel = new QLineEdit;
	formLDetalii->addRow(new QLabel("Name of med to delete:"), txtNameDel);
	btnDelete = new QPushButton("Delete med");
	formLDetalii->addWidget(btnDelete);

	txtNameUpd = new QLineEdit;
	formLDetalii->addRow(new QLabel("Name of med to update:"), txtNameUpd);
	txtNewName = new QLineEdit;
	formLDetalii->addRow(new QLabel("New name:"), txtNewName);
	txtNewProducer = new QLineEdit;
	formLDetalii->addRow(new QLabel("New producer:"), txtNewProducer);
	txtNewSubst = new QLineEdit;
	formLDetalii->addRow(new QLabel("New active substance:"), txtNewSubst);
	txtNewPrice = new QLineEdit;
	formLDetalii->addRow(new QLabel("New price:"), txtNewPrice);

	btnUpdate = new QPushButton("Update med");
	formLDetalii->addWidget(btnUpdate);

	txtNameSrch = new QLineEdit;
	formLDetalii->addRow(new QLabel("Name of med to search:"), txtNameSrch);
...
void PharmacyGUI::deleteMed() {
	try {
		ctr.deleteMed(txtNameDel->text().toStdString());
		reloadList(ctr.getAll());
...
void PharmacyGUI::updateMed() {
	try {
		ctr.updateMed(txtNameUpd->text().toStdString(), txtNewName->text().toStdString(), txtNewProducer->text().toStdString(),txtNewSubst->text().toStdString(), txtNewPrice->text().toInt());
		reloadList(ctr.getAll());
...
void PharmacyGUI::adaugaButoane(const std::vector<Meds>& pets) {
...
void PharmacyGUI::reloadList(const std::vector<Meds>& meds) {
	...
	for (const auto& m : meds) {
		QListWidgetItem* item = new QListWidgetItem(QString::fromStdString(m.getName()));
		item->setData(Qt::UserRole, QString::fromStdString(m.getProducer()));
		if (m.getPrice() > 100) {
