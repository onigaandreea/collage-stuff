...
class PendingWorker(ctx:Context, params:WorkerParameters) : CoroutineWorker(ctx, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        for(series in appContext.container.database.seriesDao().getAll()){
            if(series.requiresUpdate) {
                Log.d("PendingWorker", "REQUIRES UPDATE ${series}")
                val res = appContext.container.itemRepository.update(series)
                if(res.requiresUpdate)
                    failed++
                else successful++
            }
            else if(series.requiresCreate) {
                Log.d("PendingWorker", "REQUIRES CREATE ${series}")
                appContext.container.itemRepository.quite_remove(series)
                val res = appContext.container.itemRepository.save(series)
